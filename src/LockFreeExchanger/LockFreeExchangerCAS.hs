module LockFreeExchanger.LockFreeExchangerCAS where
...
getSlot :: IORef (Maybe a, State) -> IORef State -> IO (Maybe a)
...
newLockFreeExchanger = (newIORef (Nothing, EMPTY)) >>= return.LFE
