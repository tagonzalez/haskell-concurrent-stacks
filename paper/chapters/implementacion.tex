...
La estructura EBS fue propuesta en \cite{ebs} como una alternativa de mejor escalabilidad al LFS y luego mencionada con ejemplos de código Java en \cite{shavit}. Estos últimos ejemplos de código son la base para la implementación descripta a continuación.
...
Un \mintinline{haskell}{LockFreeExchanger} (LFE) consiste de una referencia que guarda información del estado (tipo de datos \mintinline{haskell}{State}) y un valor asociado.
El valor asociado es representado por el tipo \mintinline{haskell}{Maybe} para poder distinguir cuándo la referencia tiene un valor nulo.
...
La función se encarga de leer la tupla de la referencia \mintinline{haskell}{slot} para luego escribir su estado en la referencia \mintinline{haskell}{stampHolder} y retornar el valor de la tupla como resultado de la función.
...
Para la medición de tiempos se utilizó la función \mintinline{haskell}{getTime} de la librería \mintinline{haskell}{clock} para Haskell \cite{timeMeasuring}. El comportamiento de esta función se explica más adelante en la subsección \ref{time-measuring} del capítulo \ref{chap:experiments} de este trabajo.
...
Es importante notar que según \cite{shavit}, cada hilo de ejecución debe tener su propio \mintinline{haskell}{RangePolicy}. Para esto utilizamos la librería \mintinline{haskell}{thread-local-storage} \cite{tls-haskell} de Haskell para permitir que cada hilo tenga su propia copia de \mintinline{haskell}{RangePolicy}. Esto se muestra en la línea 1 de la figura \ref{fig:ebs-reps} dónde la componente \mintinline{haskell}{policy} tiene tipo \mintinline{haskell}{TLS RangePolicy}.
